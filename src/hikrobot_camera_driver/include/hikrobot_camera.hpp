/******************************************************************************
 * @file    hikrobot_camera.hpp
 * @author  John
 * @date    2025-03-31
 * @brief   Header file for Hikrobot camera control and configuration.
 * 
 * @details The camera class does not inherit from rclcpp::Node
 * 
 * @version 0.1
 ******************************************************************************/
#ifndef CAMERA_HPP
...
#define MAX_IMAGE_DATA_SIZE (4 * 3270 * 2048)
    ...
    Camera::Camera(int ind)
    {
        ...
        std::string config_path = current_path + "/src/hikrobot_camera_driver/config/config.yaml";
        std::cerr << "config_path: " << config_path << std::endl;
        YAML::Node config = YAML::LoadFile(config_path);
        auto camera_config = config["camera"];
        //********** 读取待设置的摄像头参数 第二个参数是默认值 yaml文件未给出该值时生效 ********************************/
        width = camera_config["width"].as<int>(1280);
        height = camera_config["height"].as<int>(1024);
        FrameRateEnable = camera_config["FrameRateEnable"].as<bool>(false);
        FrameRate = camera_config["FrameRate"].as<int>(100);
        BurstFrameCount = camera_config["BurstFrameCount"].as<int>(10);
        ExposureTime = camera_config["ExposureTime"].as<int>(4000);
        GammaEnable = camera_config["GammaEnable"].as<bool>(false);
        Gamma = camera_config["Gamma"].as<float>(0.7);
        GainAuto = camera_config["GainAuto"].as<int>(0);
        SaturationEnable = camera_config["SaturationEnable"].as<bool>(false);
        Saturation = camera_config["Saturation"].as<int>(128);
        Offset_x = camera_config["Offset_x"].as<int>(0);
        Offset_y = camera_config["Offset_y"].as<int>(0);
        TriggerMode = camera_config["TriggerMode"].as<int>(0);
        TriggerSource = camera_config["TriggerSource"].as<int>(7);
        LineSelector = camera_config["LineSelector"].as<int>(2);
        Gain = camera_config["Gain"].as<float>(20.0);
    ...
    void Camera::ReadImg(cv::Mat &image)
    {
        ...
        if (args->frame_empty)
        {
            // std::cerr << "ReadImg1" << std::endl;
            image = cv::Mat();
        }
        else
        {
            std::cerr << "ReadImg2" << std::endl;
    ...
    {
        std::cerr << "Capture thread started!" << std::endl;
        ...
        int nRet;
        unsigned char *m_pBufForDriver = (unsigned char *)malloc(sizeof(unsigned char) * MAX_IMAGE_DATA_SIZE);
        unsigned char *m_pBufForSaveImage = (unsigned char *)malloc(sizeof(unsigned char) * MAX_IMAGE_DATA_SIZE);
        ...
        while (true)
        {
            ...
            if (nRet != MV_OK)
            {
                ...
                {
                    std::cerr << "The Number of Faild Reading Exceed The Set Value!" << std::endl;
                    exit(-1);
                }
                continue;
            }
            ...
            // std::cerr << "receiving frame!" << std::endl;
